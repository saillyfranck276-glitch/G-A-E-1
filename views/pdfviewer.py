...
class PDFViewer(ft.Container):
    ...
    def __init__(self, app):
        ...
        self.base_dir = Path(getattr(self.app, "base_dir", Path(__file__).resolve().parent.parent))
        self.data_dir = Path(getattr(self.app, "data_dir", self.base_dir / "data"))
        ...
        self.pdf_pages_column = ft.Column(scroll=ft.ScrollMode.ALWAYS, expand=True, alignment=ft.MainAxisAlignment.CENTER)
    ...
    def _load_document_from_app(self):
        ...
        self.doc = getattr(self.app, "current_document", None) or \
                   getattr(self.app, "current_doc", None) or \
                   getattr(self.app, "selected_document", None)

        if not self.doc:
            self.pdf_pages_column.controls.clear()
            self.pdf_pages_column.controls.append(
                ft.Container(
                    content=ft.Text("Aucun document chargé. Double-cliquez sur une ligne depuis vos fiches de suivi.", size=14),
        ...
        pdf_direct_path = self.doc.get("pdf_path") if isinstance(self.doc, dict) else None
        
        if pdf_direct_path:
            self.doc = {"numero": Path(pdf_direct_path).stem, "externe_path": Path(pdf_direct_path)}
            self.display_pdf()
        elif self.doc:
            try:
                ...
                self.pdf_pages_column.controls.clear()
                self.pdf_pages_column.controls.append(
                    ft.Text(f"Erreur de prévisualisation : {e}", color=ft.colors.ORANGE_700)
                )
                if self.page: self.page.update()

    def _retour_intelligent(self, e):
        """Retourne dynamiquement sur la bonne vue en fonction du type de document."""
        if self.doc and isinstance(self.doc, dict) and "externe_path" in self.doc and "documents_clients" in str(self.doc["externe_path"]):
            self.app.navigate_to("Clients")
            return

        t_doc = self.doc.get("type_doc_interne") if isinstance(self.doc, dict) else None
    ...
    def get_path(self):
        ...
        if isinstance(self.doc, dict) and "externe_path" in self.doc: return Path(self.doc["externe_path"])
        
        t_doc = self.doc.get("type_doc_interne") if isinstance(self.doc, dict) else None
        if t_doc:
            if t_doc == "facture": type_folder = "Factures"
            elif t_doc == "devis": type_folder = "Devis"
            elif t_doc == "bon_commande": type_folder = "Bons_Commande"
            elif t_doc == "bon_livraison": type_folder = "Bons_Livraison"
            else: type_folder = "Autres"
        else:
            num_upper = str(self.doc.get("numero", "")).upper() if isinstance(self.doc, dict) else str(self.doc).upper()
            type_folder = "Factures" if "FACT" in num_upper or num_upper.startswith("F") else "Devis"
            
        date_str = self.doc.get("date_creation", datetime.now().strftime("%d/%m/%Y")) if isinstance(self.doc, dict) else datetime.now().strftime("%d/%m/%Y")
        annee = date_str.split("/")[-1] if "/" in date_str else datetime.now().strftime("%Y")
        
        folder = self.data_dir / "pdfs" / type_folder / annee
        folder.mkdir(parents=True, exist_ok=True)
        
        num_doc = self.doc.get('numero', 'TEMP_DOC') if isinstance(self.doc, dict) else 'TEMP_DOC'
    ...
    def display_pdf(self):
        ...
        try:
            doc = fitz.open(path)
            for page in doc:
                pix = page.get_pixmap(matrix=fitz.Matrix(self.zoom * 2, self.zoom * 2))
    ...
    def _build_interface(self):
        ...
        toolbar = ft.Row(
            controls=[
                ft.ElevatedButton("📂 Ouvrir", bgcolor=self.accent_color, color=ft.colors.WHITE, on_click=lambda _: self.file_picker.pick_files(allowed_extensions=["pdf"])),
                ft.ElevatedButton("📄 Regénérer", bgcolor="#34495E", color=ft.colors.WHITE, on_click=lambda _: self.generer_pdf_pro(silent=False)),
                ft.ElevatedButton("🖨️ Imprimer", bgcolor="#27AE60", color=ft.colors.WHITE, on_click=self.imprimer_pdf),
                ft.ElevatedButton("✍️ Signer", bgcolor="#8E44AD", color=ft.colors.WHITE, on_click=self.lancer_signature_pad),
                ft.ElevatedButton("📧 Envoyer", bgcolor="#2980B9", color=ft.colors.WHITE, on_click=self.envoyer_par_mail),
                ft.IconButton(ft.icons.ZOOM_IN_ROUNDED, icon_color=ft.colors.WHITE, bgcolor="#2C3E50", tooltip="Zoom Avant", on_click=lambda _: self.ajuster_zoom(0.15)),
                ft.IconButton(ft.icons.ZOOM_OUT_ROUNDED, icon_color=ft.colors.WHITE, bgcolor="#2C3E50", tooltip="Zoom Arrière", on_click=lambda _: self.ajuster_zoom(-0.15)),
                ft.ElevatedButton("⬅ Retour", bgcolor=ft.colors.GREY_800, color=ft.colors.WHITE, on_click=self._retour_intelligent),
            ],
            wrap=True,
            spacing=10
        )
    ...
    def on_file_picked(self, e: ft.FilePickerResultEvent):
        if e.files:
            ...
            self.doc = {"numero": Path(filepath).stem, "externe_path": Path(filepath)}
            self.zoom = 1.2
            self.display_pdf()
    ...
    def imprimer_pdf(self, e):
        ...
        if not printers:
            try:
                if sys.platform == "win32": os.startfile(str(path), "print")
                else: subprocess.run(["lpr", str(path)])
        ...
        print_dialog = ft.AlertDialog(
            ...
            content=ft.Column([ft.Text("Choisissez une machine disponible :"), dropdown_printers], tight=True),
            actions=[
                ft.TextButton("Annuler", on_click=lambda _: setattr(print_dialog, 'open', False) or self.page.update()),
                ft.ElevatedButton("Imprimer", bgcolor=self.accent_color, color=ft.colors.WHITE, on_click=valider_impression)
            ]
        )
    ...
    def executer_impression(self, path, printer_name):
        try:
            if sys.platform == "win32":
                import ctypes
                ctypes.windll.shell32.ShellExecuteW(None, "printto", str(path), f'"{printer_name}"', None, 0)
            else:
    ...
    def envoyer_par_mail(self, e):
        ...
        path = self.get_path() if hasattr(self, "get_path") else (Path(self.pdf_path) if self.pdf_path else None)
        ...
        if self.doc and isinstance(self.doc, dict):
            ...
            nom_client = c_raw.get("nom", "") if isinstance(c_raw, dict) else str(c_raw)

            full_dest_data = next((c for c in getattr(self.app, "clients", []) if str(c.get("nom")).strip().lower() == nom_client.strip().lower()), None)
            if not full_dest_data:
                full_dest_data = next((f for f in getattr(self.app, "fournisseurs", []) if str(f.get("nom")).strip().lower() == nom_client.strip().lower()), None)
        ...
        tf_to = ft.TextField(label="Destinataire", value=email_dest, placeholder="client@domaine.com")
        tf_subject = ft.TextField(label="Objet", value=f"Votre document n° {num_doc}")
        tf_body = ft.TextField(
            label="Corps du message", 
            value=f"Bonjour,\n\nVeuillez trouver ci-joint votre document n° {num_doc}.\n\nCordialement.",
            multiline=True, 
            min_lines=4, 
            max_lines=6
        )

        # 4. Éléments du formulaire de configuration SMTP (masquable)
        tf_smtp_server = ft.TextField(label="Serveur SMTP", value=smtp_conf.get("server", "smtp.gmail.com"), expand=True)
        tf_smtp_port = ft.TextField(label="Port", value=str(smtp_conf.get("port", "465")), width=100)
        tf_smtp_user = ft.TextField(label="Email expéditeur (Identifiant)", value=smtp_conf.get("user", ""))
        tf_smtp_pass = ft.TextField(label="Mot de passe d'application SMTP", value=smtp_conf.get("password", ""), password=True, can_reveal_password=True)
        cb_use_ssl = ft.Checkbox(label="Utiliser SSL / TLS (Recommandé)", value=smtp_conf.get("use_ssl", True))

        smtp_settings_layout = ft.Column([
            ft.Divider(),
            ft.Text("⚙️ Configuration Expéditeur (SMTP)", weight=ft.FontWeight.BOLD, size=12, color=ft.colors.BLUE_400),
            ft.Row([tf_smtp_server, tf_smtp_port], spacing=10),
            tf_smtp_user,
            tf_smtp_pass,
            cb_use_ssl
        ], visible=not bool(smtp_conf.get("user")), spacing=10)
        ...
        def lancer_envoi(ev):
            dest = tf_to.value.strip()
            subject = tf_subject.value.strip()
            body = tf_body.value
            
            s_server = tf_smtp_server.value.strip()
            s_port = tf_smtp_port.value.strip()
            s_user = tf_smtp_user.value.strip()
            s_pass = tf_smtp_pass.value
            s_ssl = cb_use_ssl.value

            if not dest or "@" not in dest:
                self.show_snack("L'adresse e-mail du destinataire est invalide.", is_error=True)
                return
            if not s_server or not s_port or not s_user or not s_pass:
                self.show_snack("Veuillez renseigner tous les paramètres SMTP.", is_error=True)
            ...
            # Exécution asynchrone pour ne pas bloquer l'application
        ...
        mail_dialog = ft.AlertDialog(
            title=ft.Row([
                ft.Icon(ft.icons.EMAIL_ROUNDED, color=self.accent_color or ft.colors.BLUE),
                ft.Text("Envoi du document par E-mail", size=16, weight=ft.FontWeight.BOLD)
            ], spacing=10),
            ...
            actions=[
                ft.TextButton("Annuler", on_click=lambda _: setattr(mail_dialog, 'open', False) or self.page.update()),
                ft.ElevatedButton("🚀 Envoyer", bgcolor=self.accent_color or ft.colors.BLUE, color=ft.colors.WHITE, on_click=lancer_envoi)
            ]
        )
    ...
    def lancer_signature_pad(self, e):
        ...
        def on_pan_start(ev: ft.DragStartEvent):
            ...
            self.current_path = cv.Path(
                ...
                stroke=ft.Paint(stroke_width=2.5, color=ft.colors.BLUE_900, style=ft.PaintingStyle.STROKE, stroke_cap=ft.StrokeCap.ROUND)
            )
        ...
        def on_pan_update(ev: ft.DragUpdateEvent):
            if self.last_x is not None and self.last_y is not None:
                ...
                self.canvas_control.update()
                self.draw_img.line([self.last_x, self.last_y, ev.local_x, ev.local_y], fill="#1E3A8A", width=3)
            self.last_x = ev.local_x
            self.last_y = ev.local_y
        ...
        sig_dialog = ft.AlertDialog(
            title=ft.Text("✍️ Signature Électronique", size=16, weight=ft.FontWeight.BOLD),
            content=ft.Column([
                ft.Text("Signez au doigt ou à la souris dans l'encadré :", size=12),
                ft.Container(
            ...
            actions=[
                ft.ElevatedButton("🗑️ Effacer", bgcolor=ft.colors.RED_500, color=ft.colors.WHITE, on_click=clear_canvas),
                ft.ElevatedButton("💾 Apposer", bgcolor=ft.colors.GREEN_600, color=ft.colors.WHITE, on_click=valider_signature),
            ],
    ...
    def appliquer_signature_sur_pdf(self, image_signature_path):
        ...
        try:
            ...
            if image_signature_path.exists():
                if permanent_sig_path.exists(): permanent_sig_path.unlink()
            ...
            log_text = f"Certifié conforme le {timestamp} par signature cryptographique locale."
            page.insert_text(fitz.Point(345, 770), log_text, fontsize=6, color=(0.5, 0.5, 0.5))
            ...
            self.show_snack(f"Échec du marquage de signature : {e}", is_error=True)
    ...
    def generer_pdf_pro(self, silent=False):
        ...
        if t_doc == "fiche_contact":
            ...
            c.drawString(45, height - 60, entreprise_data.get("nom", "MON ENTREPRISE").upper())
            c.setFont("Helvetica", 8)
            c.setFillColor(text_muted)
            c.drawString(45, height - 72, f"SIRET : {entreprise_data.get('siret', '-')} | CRM Interne")
            ...
            c.setFillColor(text_muted)
            c.drawString(45, height - 145, f"Date d'extraction : {datetime.now().strftime('%d/%m/%Y à %H:%M')}")
            c.drawRightString(width - 45, height - 145, f"ID Base : {self.doc.get('numero', 'N/A')}")
            
            infos_contact = [
                ("Raison Sociale / Nom", self.doc.get("nom", "-").upper(), "Helvetica-Bold", text_dark),
                ("Numéro SIRET", self.doc.get("siret", "-") or "Non renseigné", "Helvetica", text_dark),
                ("Ligne Téléphonique", self.doc.get("telephone", "-") or "Non renseigné", "Helvetica", text_dark),
                ("Courrier Électronique", self.doc.get("email", "-") or "Non renseigné", "Helvetica", text_dark),
                ("Adresse Postale", f"{self.doc.get('adresse', '-')}\n{self.doc.get('code_postal', '-')} {self.doc.get('ville', '-')}", "Helvetica", text_dark),
            ]
            ...
            if self.doc.get("statut") == "Signé" and permanent_sig_path.exists():
                try:
                    c.drawImage(str(permanent_sig_path), 350, 75, width=170, height=45, mask='auto')
                    timestamp = self.doc.get("date_signature", datetime.now().strftime("%d/%m/%Y à %H:%M:%S"))
            ...
            c.setFont("Helvetica-Oblique", 7.5)
            c.setFillColor(text_muted)
            c.drawCentredString(width / 2, 42, "Ce document contient des informations à caractère personnel protégées par le RGPD. Diffusion interdite.")
            c.save()
            if not silent: self.show_snack("Fiche d'identification client générée.")
            self.display_pdf()
            return
        ...
        if t_doc == "fiche_fournisseur":
            ...
            c.setFont("Helvetica-Oblique", 7.5)
            c.setFillColor(text_muted)
            c.drawString(345, 138, "Dossier approuvé le : .............................")
            c.drawString(345, 110, "Signature exécutoire :")
            ...
            c.line(45, 55, width - 45, 55)
            c.save()
            if not silent: self.show_snack("Dossier fournisseur synchronisé.")
            self.display_pdf()
            return
        ...
        # ==============================================================
        if t_doc:
            if t_doc == "facture": titre_doc = "FACTURE"
            elif t_doc == "devis": titre_doc = "DEVIS"
            elif t_doc == "bon_commande": titre_doc = "BON DE COMMANDE"
            elif t_doc == "bon_livraison": titre_doc = "BON DE LIVRAISON"
            else: titre_doc = str(t_doc).upper()
        else:
            is_facture = "FACT" in str(self.doc.get("numero", "")).upper() or str(self.doc.get("numero", "")).upper().startswith("F")
        ...
        def dessiner_footer(canvas_obj, current_page):
            ...
            mentions_legales = f"{nom_ent} — {statut} au capital social variable — SIRET : {siret} — {rc_pro}"
        ...
        def dessiner_entete_page(canvas_obj, page_num):
            ...
            if page_num == 1:
                ...
                canvas_obj.drawString(45, height - 45, entreprise_data.get("nom", "MON ENTREPRISE").upper())
                ...
                canvas_obj.drawString(45, y_em - 13, f"SIRET : {entreprise_data.get('siret', '-')} | TVA : {entreprise_data.get('tva_intracom', '-')}")
                canvas_obj.drawString(45, y_em - 26, f"Contact : {entreprise_data.get('email', '-')} | {entreprise_data.get('telephone', '-')}")
                ...
                label_dest = "DESTINATAIRE FOURNISSEUR" if t_doc in ["bon_commande", "bon_livraison"] else "DESTINATAIRE CLIENT"
                canvas_obj.drawString(342, height - 48, label_dest)
                
                c_raw = self.doc.get('client', self.doc.get('fournisseur', 'Client Non Référencé'))
                nom_client = c_raw.get('nom', '') if isinstance(c_raw, dict) else str(c_raw)
                ...
                c_data = full_dest_data if isinstance(full_dest_data, dict) else (c_raw if isinstance(c_raw, dict) else {"nom": nom_client})
                
                canvas_obj.setFillColor(text_dark)
                canvas_obj.setFont("Helvetica-Bold", 10.5)
                canvas_obj.drawString(342, height - 64, c_data.get('nom', 'Inconnu').upper())
                
                canvas_obj.setFont("Helvetica", 8.5)
                canvas_obj.setFillColor(text_muted)
                y_cli = height - 78
                c_adr = f"{c_data.get('adresse', '')}, {c_data.get('code_postal', '')} {c_data.get('ville', '')}".strip(", ")
                if c_adr: canvas_obj.drawString(342, y_cli, c_adr[:45])
                if c_data.get('siret'): canvas_obj.drawString(342, y_cli - 13, f"SIRET : {c_data['siret']}")

                # Bloc Infos Titre Document 
                canvas_obj.setFillColor(text_dark)
                canvas_obj.setFont("Helvetica-Bold", 16)
                canvas_obj.drawString(45, height - 150, f"{titre_doc} # {self.doc.get('numero', 'N/A')}")
                ...
                canvas_obj.setFont("Helvetica", 8.5)
                canvas_obj.setFillColor(text_muted)
                date_doc = self.doc.get("date_creation", datetime.now().strftime("%d/%m/%Y"))
                canvas_obj.drawString(45, height - 173, f"Date d'émission : {date_doc}")
                
                date_ech = self.doc.get("date_echeance", "À réception")
                canvas_obj.drawRightString(width - 45, height - 173, f"Date limite de règlement : {date_ech}")
                
                statut_doc = str(self.doc.get("statut", ""))
                if titre_doc == "FACTURE" and any(x in statut_doc.lower() for x in ["payé", "déclaré"]):
                ...
                canvas_obj.drawString(45, height - 30, f"{titre_doc} n° {self.doc.get('numero', 'N/A')} — Page {page_num}")
                canvas_obj.setStrokeColor(border_color)
        ...
        def dessiner_entete_tableau(canvas_obj, y_pos):
            ...
            canvas_obj.drawString(52, y_pos + 6, "Description de la prestation / produit")
        ...
        if not items_list:
            items_list = [{"designation": "Prestation de services standard", "qty": 1, "pu": float(self.doc.get("total_ttc", self.doc.get("montant_ht", self.doc.get("montant", 0)))) or 0.0}]
        ...
        for idx, item in enumerate(items_list):
            ...
            for mot in mots:
                if len(ligne_courante + mot) <= 62: 
                    ...
                    ligne_courante = mot + " "
            if ligne_courante: lignes_desc.append(ligne_courante.strip())
            ...
            if idx % 2 == 1:
                ...
                c.rect(45, y_items - hauteur_totale_bloc + 4, width - 90, hauteur_totale_bloc + 2, stroke=0, fill=1)
            
            c.setFillColor(text_dark)
            c.setFont("Helvetica", 9)
            
            try: qty = int(item.get("qty", item.get("quantite", item.get("qte", 1))))
            except: qty = 1
            try: pu = float(item.get("pu", item.get("prix", item.get("prix_unitaire", 0.0))))
            except: pu = 0.0
        ...
        c.drawString(350, y_bloc_total - 15, f"Taxe sur la Valeur Ajoutée ({tva_label})")
        ...
        c.setFont("Helvetica-Oblique", 7.5)
        if tva_doc == 0:
            c.drawString(45, y_bloc_total - 35, str(entreprise_data.get("mention_tva", "TVA non applicable, art. 293 B du CGI")))

        c.setFont("Helvetica-Bold", 8)
        c.setFillColor(text_dark)
        c.drawString(45, y_bloc_total - 65, "CONDITIONS DE RÈGLEMENT COMMERCIALES & LÉGALES :")
        ...
        lignes_legales = [
            f"Règlement par virement bancaire sur compte d'exploitation courant : IBAN {iban} — BIC {bic}",
            "Pas d'escompte consenti pour versement anticipé. En cas de retard de paiement de la présente facture à l'échéance,",
            "il sera appliqué de plein droit des pénalités de retard calculées sur la base de 3 fois le taux d'intérêt légal en vigueur.",
            "Indemnité forfaitaire légale pour frais de recouvrement obligatoire en cas de retard : 40,00 € (Art. L441-6 / Code de Com.)."
        ]
        ...
        c.drawString(345, 126, "Précédée de la date du jour : ...................................")
        c.drawString(345, 110, "Signature exécutoire :")
    ...
    def show_snack(self, message, is_error=False):
        """Notification Snack-bar Flet moderne."""
