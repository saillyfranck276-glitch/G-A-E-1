...
class FournisseursView(ft.Column):
    def __init__(self, app):
        ...
        # 🚀 Sécurité Android/Cross-platform : Initialisation du sélecteur de fichiers natif
        self.file_picker = ft.FilePicker(on_result=self.on_file_picker_result)
        if self.page.overlay:
            self.page.overlay.append(self.file_picker)
        else:
            self.page.overlay = [self.file_picker]

        self.setup_ui()

    def setup_ui(self):
        """Crée l'en-tête et les onglets segmentés d'origine"""
        # En-tête principal
        self.controls.append(
            ft.Row(
                controls=[
                    ft.Text("📦 Chaîne Logistique : Fournisseurs & Achats", size=24, weight=ft.FontWeight.BOLD)
                ],
                alignment=ft.MainAxisAlignment.START
            )
        )

        # 🔘 Système d'onglets (Modèle CTkSegmentedButton à l'identique)
        self.tab_view = ft.SegmentedButton(
            segments=[
                ft.Segment(value="repertoire", label=ft.Text("🏢 Répertoire Fournisseurs", size=12)),
                ft.Segment(value="achats", label=ft.Text("📄 Suivi des Achats (BC / BL)", size=12)),
            ],
            selected={"repertoire"},
    ...
    def _afficher_fournisseurs(self):
        ...
        self.lbl_form_title = ft.Text("📝 Ajouter un nouveau partenaire", size=14, weight=ft.FontWeight.BOLD, color=self.accent_color)
        form_column.controls.append(self.lbl_form_title)
        
        for key, label, placeholder in fields_config:
            form_column.controls.append(ft.Text(label, size=11, weight=ft.FontWeight.BOLD, color="#AEAEB2"))
            tf = ft.TextField(hint_text=placeholder, bgcolor="#242426", border_color="#2A2A2E", height=36, text_size=12, content_padding=10)
            self.entries[key] = tf
            form_column.controls.append(tf)
            
        self.lbl_status = ft.Text("", size=11, italic=True, color=ft.colors.RED_400)
        ...
        self.list_title = ft.Text(f"Fiches partenaires enregistrées ({count_fourn})", size=13, weight=ft.FontWeight.BOLD, color="#AEAEB2")
        
        right_frame = ft.Column(
            controls=[
                self.search_entry,
                self.list_title,
                ft.Container(content=self.list_column, expand=True, bgcolor="#141416", border_radius=8, padding=10)
            ],
            expand=True
        )
    ...
    def refresh_fournisseur_list(self, e):
        """Régénère la liste filtrée à l'identique"""
        search_term = self.search_entry.value.lower().strip() if self.search_entry.value else ""
        ...
        if not fournisseurs:
            self.list_column.controls.append(
                ft.Text("Aucun fournisseur dans votre base de données.", size=13, italic=True, color="#636366")
            )
            self.page.update()
            return
        ...
        for idx, f in enumerate(fournisseurs):
            ...
            if search_term and not (search_term in nom or search_term in contact or search_term in tel or search_term in email):
                continue
                
            visible_fourn += 1
            
            if "documents_externes" not in f or not isinstance(f["documents_externes"], list):
                f["documents_externes"] = []
                
            # Texte des coordonnées descriptives
            details_text = f"👤 Contact : {f.get('contact_nom') or 'Non renseigné'}   |   📞 {f.get('telephone') or 'Non renseigné'}\n" \
                           f"✉️ {f.get('email') or 'Non renseigné'}   |   📍 {f.get('adresse') or '-'} {f.get('code_postal') or ''} {f.get('ville') or ''}"
                ...
                controls=[
                    ft.Row(
                        controls=[
                            ft.Column(
                                controls=[
                                    ft.Text(f["nom"].upper(), size=13, weight=ft.FontWeight.BOLD, color="#38BDF8"),
                            ...
                            ft.Column(
                                controls=[
                                    ft.ElevatedButton("✏️ Modifier", bgcolor=self.accent_color, color=ft.colors.WHITE, height=28, text_size=11, on_click=lambda e, i=idx, item=f: self.charger_fournisseur(i, item)),
                                    ft.ElevatedButton("👁️ Fiche", bgcolor="#6B7280", color=ft.colors.WHITE, height=28, text_size=11, on_click=lambda e, item=f: self._voir_fiche_interne(item)),
                                    ft.ElevatedButton(f"📁 Docs ({len(f['documents_externes'])})", bgcolor="#10B981", color=ft.colors.WHITE, height=28, text_size=11, on_click=lambda e, i=idx: self.toggle_documents_panel(i)),
                                    ft.ElevatedButton("🗑️ Supprimer", bgcolor="#991B1B", color=ft.colors.WHITE, height=28, text_size=11, on_click=lambda e, item=f: self._supprimer_fournisseur(item)),
                                ],
        ...
        if visible_fourn == 0 and search_term:
            self.list_column.controls.append(
                ft.Text("🔍 Aucun fournisseur ne correspond à votre recherche.", size=12, italic=True, color="#636366")
            )
            
        self.list_title.value = f"Fiches partenaires enregistrées ({len(fournisseurs)})"
        self.page.update()

    # ─────────────────────────────────────────────────────────────────────────
    # 📁 VOLET DES PIÈCES JOINTES (Modèle intelligent imbriqué)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def populate_docs_panel(self, container_col, fournisseur, fourn_idx):
        """Remplace render_documents_rows de Tkinter de façon moderne"""
        categories = ["RIB / Banque", "Contrat Commercial", "Extrait Kbis", "Devis Référence", "Autre Document"]
        ...
        btn_add = ft.ElevatedButton(
            ...
            height=32,
            text_size=11,
            on_click=lambda e: self.declencher_ajout_pdf(fournisseur, fourn_idx, cat_combo.value)
        )
        ...
        if not fournisseur["documents_externes"]:
            rows_col.controls.append(
                ft.Text("Aucun document rattaché à ce fournisseur.", size=11, italic=True, color="#636366")
            )
        else:
            for doc_idx, doc in enumerate(fournisseur["documents_externes"]):
                colors = {"RIB / Banque": "#3B82F6", "Contrat Commercial": "#10B981", "Extrait Kbis": "#F59E0B", "Devis Référence": "#8B5CF6"}
                bg_color = colors.get(doc['type'], "#6B7280")
                
                badge = ft.Container(
                    content=ft.Text(f" {doc['type'].upper()} ", size=9, weight=ft.FontWeight.BOLD, color=ft.colors.WHITE),
                    bgcolor=bg_color,
                    border_radius=4,
                    padding=3
                )
                ...
                row = ft.Row(
                    controls=[
                        badge,
                        ft.Text(doc["nom_fichier"], size=11, color="#E5E7EB", expand=True),
                        ft.IconButton(ft.icons.REMOVE_RED_EYE_OUTLINED, icon_size=16, icon_color="#E5E7EB", on_click=lambda e, d=doc: self.ouvrir_pdf(d)),
                        ft.IconButton(ft.icons.DELETE_OUTLINE, icon_size=16, icon_color=ft.colors.RED_400, on_click=lambda e, fourn=fournisseur, fi=fourn_idx, di=doc_idx: self.supprimer_pdf_fournisseur(fourn, fi, di))
                    ],
                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN
                )
        ...
        container_col.controls.extend([
            ...
            ft.Text("📁 Pièces jointes & Classement PDF Fournisseur", size=11, weight=ft.FontWeight.BOLD, color="#A7F3D0"),
            tools_row,
            rows_col
        ])
    ...
    def on_file_picker_result(self, e: ft.FilePickerResultEvent):
        """Gère physiquement la copie du fichier sur disque de manière unifiée"""
        ...
        try:
            ...
            base_dir = Path(getattr(self.app, "data_dir", "data")) / "documents_fournisseurs"
            
            nom_propre = "".join(c for c in self.current_upload_fournisseur.get("nom", "Inconnu") if c.isalnum() or c in (" ", "_", "-")).strip().replace(" ", "_")
            ...
            self.show_error_message("Erreur d'importation", f"Impossible d'importer le document :\n{ex}")
        finally:
            self.current_upload_fournisseur = None
    ...
    def supprimer_pdf_fournisseur(self, fournisseur, fourn_idx, doc_idx):
        ...
        def confirm():
            try:
                ...
                self.show_error_message("Erreur de suppression", f"Une erreur est survenue :\n{e}")
                
        self.show_confirm_dialog(
            "Confirmation", 
            f"Voulez-vous vraiment détruire et détacher le document '{doc['nom_fichier']}' ?", 
            confirm
        )
    ...
    def ouvrir_pdf(self, doc):
        try:
            ...
            if file_path.exists():
                ...
                self.show_error_message("Fichier introuvable", "Le fichier physique est introuvable ou a été déplacé.")
        except Exception as e:
            self.show_error_message("Erreur d'ouverture", f"Impossible d'ouvrir le document :\n{e}")

    def _voir_fiche_interne(self, f):
        donnees_fiche = {
            "type_doc_interne": "fiche_fournisseur",
            "type_contact": "fournisseur",
            "numero": f"FICHE-FOUR-{str(f.get('nom', 'INCONNU')).upper().replace(' ', '_')[:12]}",
            "date_creation": "-",
            "nom": f.get("nom", ""),
        ...
        self.app.navigate_to("PDFViewer", doc_type="fiche_fournisseur", doc_to_edit=donnees_fiche)

    # ─────────────────────────────────────────────────────────────────────────
    # LOGIQUE FORMULAIRE ET SYNCRONISATION
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def vider_champs(self, e=None):
        ...
        self.btn_save.bgcolor = ft.colors.GREEN_700
    ...
    def _valider_fournisseur(self, e):
        ...
        if self.editing_idx is not None:
            for key in self.entries:
                fournisseurs[self.editing_idx][key] = self.entries[key].value.strip() if self.entries[key].value else ""
        else:
            fourn_data = {key: (self.entries[key].value.strip() if self.entries[key].value else "") for key in self.entries}
    ...
    def _supprimer_fournisseur(self, f):
        ...
        self.show_confirm_dialog(
            "Confirmation", 
            f"Supprimer définitivement le partenaire '{f['nom']}' et TOUTES ses pièces jointes ?", 
            confirm
        )
    ...
    def _afficher_documents_achat(self):
        toolbar = ft.Row(
            controls=[
                ft.ElevatedButton("➕ Nouveau Bon de Commande (BC)", bgcolor=self.accent_color, color=ft.colors.WHITE, on_click=lambda e: self.app.navigate_to("CreateDocument", doc_type="bon_commande")),
                ft.ElevatedButton("➕ Nouveau Bon de Livraison (BL)", bgcolor=self.accent_color, color=ft.colors.WHITE, on_click=lambda e: self.app.navigate_to("CreateDocument", doc_type="bon_livraison")),
            ],
            spacing=10
        )
        
        # 📊 Remplacement parfait de Treeview par ft.DataTable (avec sélection sécurisée)
        ...
        table_container = ft.Container(
            content=ft.Column(controls=[self.tree_table], scroll=ft.ScrollMode.AUTO),
            bgcolor="#141416",
            border_radius=12,
        ...
        actions_bar = ft.Row(
            controls=[
                ft.ElevatedButton("👁️ Voir le PDF", bgcolor=self.accent_color, color=ft.colors.WHITE, on_click=self._voir_pdf_selectionne),
                ft.OutlinedButton("✏️ Modifier la pièce", on_click=self._modifier_document_selectionne),
                ft.ElevatedButton("🗑️ Supprimer", bgcolor="#991B1B", color=ft.colors.WHITE, on_click=self._supprimer_document_selectionne),
            ],
            spacing=10
        )
    ...
    def _select_row(self, e, doc_type, doc_data):
        """Gère la surbrillance de ligne active au clic"""
    ...
    def _charger_pieces_achat(self):
        ...
        for bc in getattr(self.app, "bons_commande", []):
            ...
            f_nom = f_info.get("nom", "Inconnu") if isinstance(f_info, dict) else str(f_info)
    ...
    def _voir_pdf_selectionne(self, e):
        if not self.selected_row_data:
            self.show_error_message("Sélection requise", "Veuillez sélectionner un document d'achat à visualiser.")
            return
    ...
    def _modifier_document_selectionne(self, e):
        if not self.selected_row_data:
            self.show_error_message("Sélection requise", "Veuillez sélectionner un document d'achat à modifier.")
            return
        doc_type, doc_data = self.selected_row_data
        self.app.navigate_to("CreateDocument", doc_type=doc_type, doc_to_edit=doc_data)

    def _supprimer_document_selectionne(self, e):
        if not self.selected_row_data:
            self.show_error_message("Sélection requise", "Veuillez sélectionner une pièce à supprimer.")
            return
        doc_type, doc_data = self.selected_row_data
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # MODALS ET DIALOGUES NATIFS (Remplacement MessageBox)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def show_error_message(self, title, text):
        ...
        dlg = ft.AlertDialog(
            title=ft.Text(title, color=ft.colors.RED_400),
            content=ft.Text(text),
            actions=[
